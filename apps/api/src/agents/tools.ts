...
function extractStringArg(input: any, fieldName = 'input'): string {
  ...
  if (input && typeof input === 'object') {
    return input[fieldName] || input.query || input.city_name || input.city || JSON.stringify(input);
  }
  return String(input);
}
...
const listAgentCapabilities = new DynamicTool({
  ...
  description: 'Returns a complete list of all weather tools available to this agent and what each one does. Use this when the user asks "what can you do?" or wants to know your capabilities.',
  func: async () => {
...
const getUserCities = (userId: string) =>
  new DynamicTool({
    ...
    description: 'Returns all cities the user has saved on their dashboard, including their favorite status.',
    func: async () => {
      const cities = await City.find({ userId }).sort({ createdAt: -1 });
      if (!cities.length) return 'The user has no cities saved yet. Suggest they add some from the dashboard.';
...
const getWeatherCurrent = (userId: string) =>
  new DynamicTool({
    ...
    description: 'Fetches current weather for a saved city by name. Returns temperature, condition, humidity, wind speed, and precipitation. Input: city name.',
    func: async (input: any) => {
      ...
      if (!city) return `City "${city_name}" not found in the user's dashboard. Use search_city_weather to look up any city worldwide.`;
...
const getWeatherForecast = (userId: string) =>
  new DynamicTool({
    ...
    description: 'Fetches a 7-day daily weather forecast for a saved city. Returns day-by-day highs, lows, conditions, and precipitation. Input: city name.',
    func: async (input: any) => {
      const city_name = extractStringArg(input, 'city_name');
      const city = await City.findOne({
        userId,
...
const getWeatherStreak = (userId: string) =>
  new DynamicTool({
    ...
    description: 'Analyzes the last 15 days of weather for a saved city and reports any consecutive-day streak (e.g., "Rainy for 5 days", "Sunny streak"). Input: city name.',
    func: async (input: any) => {
      const city_name = extractStringArg(input, 'city_name');
      const city = await City.findOne({
        userId,
      ...
      if (!streak) return `No notable weather streak for ${city.name} right now. Conditions have been mixed over the past two weeks.`;
...
const searchCityWeather = new DynamicTool({
  ...
  description: 'Searches for any city worldwide by name, geocodes it, and returns current weather. Use this when the user asks about a city they have NOT saved in their dashboard. Input: city name or query.',
  func: async (input: any) => {
...
const getCalendarWeatherAlerts = (userId: string) =>
  new DynamicTool({
    ...
    description: 'Returns upcoming calendar events that have weather alerts attached. Shows event title, date, location, and the weather warning message.',
    func: async () => {
      const alerts = await CalendarAlert.find({ userId }).sort({ eventStart: 1 }).limit(10);
      if (!alerts.length) return 'No upcoming calendar weather alerts. Connect your Google Calendar in Settings to get alerts.';
...
const compareCities = (userId: string) =>
  new DynamicTool({
    name: 'compare_cities',
    description: 'Compares current weather across multiple saved cities at once. Input: array of city names (e.g., ["Mumbai", "Delhi", "Shimla"]).',
    func: async (input: any) => {
      const raw = extractStringArg(input, 'city_names');
      const cityNames = Array.isArray(raw) ? raw : raw.split(',').map((n: string) => n.trim()).filter(Boolean);
