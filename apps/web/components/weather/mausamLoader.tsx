  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          bottom: 10, right: 10, width: 18, height: 18,
      ...
      <div
        className="absolute"
        style={{ bottom: 16, animation: "mausamCloud 5s linear infinite" }}
      >
        <CloudShape
          baseW={54} baseH={18} color="rgba(255,255,255,0.88)"
          bumps={[
...
function CloudShape({
  ...
  baseW: number; baseH: number; color: string;
  bumps: { w: number; h: number; top: number; left: number }[];
}) {
  return (
    <div style={{ position: "relative", width: baseW, height: baseH, background: color, borderRadius: 30 }}>
      {bumps.map((b, i) => (
        <div key={i} style={{
          position: "absolute", width: b.w, height: b.h,
          top: b.top, left: b.left, background: color, borderRadius: "50%",
        }} />
      ))}
    </div>
  );
...
function DefaultLoader({ hint }: { hint: string }) {
  ...
  useEffect(() => {
    ...
    [
      { ref: c1Ref, w: 90, h: 30, color: "rgba(255,255,255,0.9)", bumps: [{ w: 44, h: 40, top: -22, left: 10 }, { w: 36, h: 32, top: -16, left: 46 }] },
      { ref: c2Ref, w: 70, h: 22, color: "rgba(200,220,255,0.7)", bumps: [{ w: 34, h: 30, top: -16, left: 8  }, { w: 26, h: 24, top: -12, left: 36 }] },
      { ref: c3Ref, w: 55, h: 18, color: "rgba(255,255,255,0.75)", bumps: [{ w: 28, h: 24, top: -12, left: 6  }, { w: 20, h: 18, top: -8,  left: 30 }] },
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 py-12">
      ...
      <div
        ...
        style={{ background: "linear-gradient(180deg,#0a1628 0%,#1a3a5c 40%,#2563eb 100%)" }}
      >
        {/* Sun */}
        <div
          className="absolute rounded-full"
          style={{
            bottom: 22, right: 28, width: 32, height: 32,
            background: "radial-gradient(circle,#fef3c7 30%,#fcd34d 60%,#f59e0b 100%)",
            animation: "mausamSun 3s ease-in-out infinite",
          }}
        />
        {/* Clouds */}
        <div ref={c1Ref} className="absolute" style={{ bottom: 38, animation: "mausamCloud 8s linear infinite", animationDelay: "-1s" }} />
        <div ref={c2Ref} className="absolute" style={{ bottom: 54, animation: "mausamCloud 11s linear infinite", animationDelay: "-4s", opacity: 0.75 }} />
        <div ref={c3Ref} className="absolute" style={{ bottom: 28, animation: "mausamCloud 6.5s linear infinite", animationDelay: "-6s", opacity: 0.6 }} />
        {/* Rain */}
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="font-playfair text-[22px] font-black tracking-tight text-[var(--text-primary,#0f172a)]">Mausam</p>
        <ProgressBar />
        <Dots />
  ...
  return (
    <div className="flex items-center gap-4 py-4 px-2">
      ...
      <div className="flex flex-col gap-1.5 flex-1 min-w-0">
        <p className="font-playfair font-black text-[15px] text-[var(--text-primary,#0f172a)] leading-none">Mausam</p>
        <ProgressBar />
...
function FullscreenLoader() {
  return (
    <div
      ...
      style={{ background: "linear-gradient(180deg,#0a1628 0%,#1a3a5c 40%,#2563eb 80%,#93c5fd 100%)" }}
    >
      <RainCanvas count={70} />
      {/* Pulse ring */}
      <div className="relative w-20 h-20 flex items-center justify-center">
        ...
        <div className="absolute inset-0 rounded-full border-2 border-sky-300/50"
          style={{ animation: "mausamRing 1.8s ease-out infinite", animationDelay: "0.6s" }} />
        <div className="w-10 h-10 rounded-full z-10"
          style={{ background: "radial-gradient(circle,#fef3c7 30%,#fcd34d 60%,#f59e0b 100%)", animation: "mausamSun 3s ease-in-out infinite" }} />
      </div>
    ...
    <div className="flex flex-col gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100 w-full max-w-xs">
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col gap-1.5 flex-1">
          ...
          <div className="h-2 bg-slate-200 rounded-full animate-pulse" style={{ width: "45%", animationDelay: "150ms" }} />
        </div>
      </div>
      <div className="h-2 bg-slate-200 rounded-full animate-pulse" style={{ width: "90%", animationDelay: "100ms" }} />
      <div className="h-2 bg-slate-200 rounded-full animate-pulse" style={{ width: "60%", animationDelay: "250ms" }} />
    </div>
  );
}
